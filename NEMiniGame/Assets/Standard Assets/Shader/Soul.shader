...
fixed4 frag(v2f i) : SV_Target
{
	...
	float intersect = pow((1 - diff), _Pow) * _IntersectPower;
	...
	fixed4 color2 = fixed4(ambient + _LightColor0.rgb * saturate(dot(normal, worldLightDir)),1);

	fixed4 col = _Color * glow + lerp(color,color2, _Transparent);
	return col;
